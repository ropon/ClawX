...
# Post-installation script for OneClaw on Linux
...
# Create symbolic link for OneClaw app binary
if [ -x /opt/OneClaw/oneclaw ]; then
    ln -sf /opt/OneClaw/oneclaw /usr/local/bin/oneclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/OneClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/OneClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/OneClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/OneClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/oneclaw'
...
echo "OneClaw has been installed successfully."
