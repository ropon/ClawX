...
export const useSkillsStore = create<SkillsState>((set, get) => ({
  ...
  searchSkills: async (query: string) => {
    ...
    try {
      const result = await invoke('clawhub:search', { query }) as { success: boolean; results?: MarketplaceSkill[]; error?: string };
      if (result.success) {
  ...
  installSkill: async (slug: string, version?: string) => {
    ...
    try {
      const result = await invoke('clawhub:install', { slug, version }) as { success: boolean; error?: string };
      if (!result.success) {
  ...
  uninstallSkill: async (slug: string) => {
    set((state) => ({ installing: { ...state.installing, [slug]: true } }));
    try {
      const result = await invoke('clawhub:uninstall', { slug }) as { success: boolean; error?: string };
      if (!result.success) {
