...
interface SyncProxyOptions {
   ...
   * currently disabled in OneClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync OneClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because OneClaw proxy is disabled and preserve mode is enabled');
      return;
    }
