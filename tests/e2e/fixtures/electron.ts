...
async function launchOneClawElectron(
  homeDir: string,
  userDataDir: string,
  ...
  return await electron.launch({
    ...
    env: {
      ...
      ONECLAW_E2E: '1',
      ONECLAW_USER_DATA_DIR: userDataDir,
      ...(options.skipSetup ? { ONECLAW_E2E_SKIP_SETUP: '1' } : {}),
      ONECLAW_PORT_ONECLAW_HOST_API: String(hostApiPort),
    },
    timeout: 90_000,
  });
}
...
export const test = base.extend<ElectronFixtures>({
  homeDir: async ({ browserName: _browserName }, provideHomeDir) => {
    const homeDir = await mkdtemp(join(tmpdir(), 'oneclaw-e2e-home-'));
  ...
  userDataDir: async ({ browserName: _browserName }, provideUserDataDir) => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'oneclaw-e2e-user-data-'));
    try {
  ...
  launchElectronApp: async ({ homeDir, userDataDir }, provideLauncher) => {
    await provideLauncher(async (options?: LaunchElectronOptions) => await launchOneClawElectron(homeDir, userDataDir, options));
  },
