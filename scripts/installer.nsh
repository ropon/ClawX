; OneClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting OneClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers OneClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if OneClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old OneClaw was installed
  ...
  ; "Can't modify OneClaw's files" retry loop -> "OneClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "OneClaw 无法关闭" dialog.
...
; "OneClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with OneClaw's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove OneClaw application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\oneclaw (local app data)$\r$\n  • AppData\Roaming\oneclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering OneClaw processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\oneclaw"
    RMDir /r "$APPDATA\oneclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\oneclaw
    IfFileExists "$LOCALAPPDATA\oneclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\oneclaw"
      IfFileExists "$LOCALAPPDATA\oneclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\oneclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\oneclaw
    IfFileExists "$APPDATA\oneclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\oneclaw"
      IfFileExists "$APPDATA\oneclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\oneclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\oneclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\oneclaw"
    IfFileExists "$APPDATA\oneclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\oneclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\oneclaw"
    RMDir /r "$R3\AppData\Roaming\oneclaw"
