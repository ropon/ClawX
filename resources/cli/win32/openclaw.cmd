...
if /i "%1"=="update" (
    echo openclaw is managed by OneClaw ^(bundled version^).
    echo.
    echo To update openclaw, update OneClaw:
    echo   Open OneClaw ^> Settings ^> Check for Updates
    echo   Or download the latest version from https://oneclaw.net
    exit /b 0
)
...
set OPENCLAW_EMBEDDED_IN=OneClaw
...
if "%_USE_BUNDLED_NODE%"=="1" (
    ...
    "%~dp0..\..\OneClaw.exe" "%OPENCLAW_ENTRY%" %*
)
