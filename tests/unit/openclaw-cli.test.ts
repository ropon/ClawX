...
vi.mock('@electron/utils/paths', () => ({
  ...
  getOpenClawEntryPath: () => 'C:\\Program Files\\OneClaw\\resources\\openclaw\\openclaw.mjs',
}));
...
describe('getOpenClawCliCommand (Windows packaged)', () => {
  beforeEach(() => {
    ...
    Object.defineProperty(process, 'resourcesPath', {
      value: 'C:\\Program Files\\OneClaw\\resources',
      configurable: true,
      writable: true,
    });
  });
  ...
  it('prefers bundled node.exe when present', async () => {
    ...
    expect(getOpenClawCliCommand()).toBe(
      "& 'C:\\Program Files\\OneClaw\\resources/cli/openclaw.cmd'",
    );
  });
  ...
  it('falls back to bundled node.exe when openclaw.cmd is missing', async () => {
    ...
    const { getOpenClawCliCommand } = await import('@electron/utils/openclaw-cli');
    expect(getOpenClawCliCommand()).toBe(
      "& 'C:\\Program Files\\OneClaw\\resources/bin/node.exe' 'C:\\Program Files\\OneClaw\\resources\\openclaw\\openclaw.mjs'",
    );
  });
  ...
  it('falls back to ELECTRON_RUN_AS_NODE command when wrappers are missing', async () => {
    ...
    expect(command.endsWith("'C:\\Program Files\\OneClaw\\resources\\openclaw\\openclaw.mjs'")).toBe(true);
  });
});
