...
Use this spec when OneClaw shows the Gateway as starting/running but UI data does not refresh, Dreams cannot load, or Gateway RPC calls time out after a restart.

OneClaw should prefer OpenClaw-native signals over stderr string matching:
...
- `gateway.ready`, `health`, and `presence` events should update OneClaw's main-process capability cache.
...
- **Handshake ready** only means OneClaw connected to the Gateway socket.
...
2. Read recent OneClaw logs:

```bash
tail -n 160 "$HOME/Library/Application Support/oneclaw/logs/oneclaw-$(date +%F).log"
```
...
```bash
pnpm exec openclaw gateway call system-presence >/tmp/oneclaw-system-presence.json
pnpm exec openclaw gateway call health --params '{"probe":false}' >/tmp/oneclaw-health.json
pnpm exec openclaw gateway call status >/tmp/oneclaw-status.json
pnpm exec openclaw gateway call channels.status --params '{"probe":false}' >/tmp/oneclaw-channels-status.json
pnpm exec openclaw gateway call doctor.memory.status >/tmp/oneclaw-memory-status.json
pnpm exec openclaw gateway call doctor.memory.dreamDiary >/tmp/oneclaw-dream-diary.json
```
...
```bash
sample <gateway-pid> 3 -mayDie >/tmp/oneclaw-gateway.sample.txt
```
...
- Where the artifact will be written, usually `/tmp/oneclaw-gateway.sample.txt`.
- What will be inspected and what will not be shared verbatim.

Do not proceed without explicit user agreement when:

- Sampling a process that is not clearly the OneClaw-owned Gateway child.
...
- Whether the finding points to OneClaw-owned prelaunch cleanup, OpenClaw runtime startup cost, active user work, or inconclusive data.
...
- Explain to users that restart cost is dominated by active Gateway work, not by OneClaw UI rendering.
...
- Logs no longer repeat stale runtime cache or escaped managed-skill symlink warnings for entries OneClaw can safely clean.
...
- Separate OneClaw-owned cleanup issues from OpenClaw runtime initialization cost.
