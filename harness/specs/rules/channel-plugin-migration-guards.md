...
When channel plugin ownership changes between bundled OpenClaw extensions and external `~/.openclaw/extensions/*` installs, OneClaw must normalize configuration to one active plugin identity per channel.
...
- when the canonical Feishu plugin is external, OneClaw must explicitly disable the bundled `feishu` plugin instead of only removing allowlist entries
