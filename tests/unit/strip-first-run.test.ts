...
const { testHome } = vi.hoisted(() => ({
  testHome: `/tmp/oneclaw-openclaw-workspace-${Math.random().toString(36).slice(2)}`,
}));
...
describe('stripFirstRunSection', () => {
  ...
  it('still changes AGENTS content when only First Run is removed', () => {
    ...
    const merged = mergeOneClawSection(stripped, section);

    expect(merged).not.toBe(original);
    expect(merged).not.toContain('## First Run');
    expect(merged).toContain('## Session Startup');
    expect(merged).toContain('<!-- oneclaw:begin -->');
    expect(merged).toContain('<!-- oneclaw:end -->');
  });
});

describe('ensureOneClawIdentityFile', () => {
  it('writes a default OneClaw identity when the workspace has none', async () => {
    const workspaceDir = join(testHome, '.openclaw', 'workspace');
    await mkdir(workspaceDir, { recursive: true });

    await ensureOneClawIdentityFile(workspaceDir);

    await expect(readFile(join(workspaceDir, 'IDENTITY.md'), 'utf-8')).resolves.toContain('OneClaw');
  });
...
describe('ensureOneClawDefaultIdentity', () => {
  it('creates the default workspace and seeds IDENTITY.md for startup-owned workspaces', async () => {
    await ensureOneClawDefaultIdentity();

    await expect(readFile(join(testHome, '.openclaw', 'workspace', 'IDENTITY.md'), 'utf-8')).resolves.toContain('OneClaw');
  });
});

describe('ensureOneClawContext', () => {
  it('does not wait for missing files in non-default agent workspaces', async () => {
    ...
    const result = await Promise.race([
      ensureOneClawContext().then(() => 'done'),
      new Promise((resolve) => setTimeout(() => resolve('timeout'), 200)),
    ]);

    expect(result).toBe('done');
    await expect(readFile(join(defaultWorkspace, 'AGENTS.md'), 'utf-8')).resolves.toContain('## OneClaw Environment');
    await expect(readFile(join(defaultWorkspace, 'TOOLS.md'), 'utf-8')).resolves.toContain('## OneClaw Tool Notes');
