...
import {
  ensureOneClawContext,
  ensureOneClawDefaultIdentity,
  repairOneClawOnlyBootstrapFiles,
...
const WINDOWS_APP_USER_MODEL_ID = 'app.oneclaw.desktop';
const isE2EMode = process.env.ONECLAW_E2E === '1';
const requestedUserDataDir = process.env.ONECLAW_USER_DATA_DIR?.trim();
...
// On Wayland this maps the running window to oneclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('oneclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[OneClaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[OneClaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[OneClaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
function createWindow(): BrowserWindow {
  ...
  const shouldSkipSetupForE2E = process.env.ONECLAW_E2E_SKIP_SETUP === '1';
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== OneClaw Application Starting ===');
  logger.debug(
  ...
  // workspace so OneClaw desktop sessions skip OpenClaw's chat-first bootstrap.
  if (!isE2EMode) {
    void ensureOneClawDefaultIdentity().catch((error) => {
      logger.warn('Failed to seed default OneClaw identity:', error);
    });
  }

  // Repair any bootstrap files that only contain OneClaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureOneClawContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairOneClawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureOneClawContext().catch((error) => {
        logger.warn('Failed to re-merge OneClaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge OneClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureOneClawContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureOneClawContext().catch((error) => {
      logger.warn('Failed to merge OneClaw context into workspace:', error);
    });
  }
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second OneClaw instance detected; redirecting to the existing window');
