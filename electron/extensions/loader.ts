...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'oneclaw-extensions.json');
  }
  return join(app.getAppPath(), 'oneclaw-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No oneclaw-extensions.json found, loading all builtin extensions');
  }
